...
- настраиваемое автозакрытие регистрации (глобально, по умолчанию 24 часа с момента открытия).
...
- Автозакрытие регистрации задается в админке (`Настройки`):
  - значение по умолчанию: `24` часа;
  - логика: таймер идет от момента открытия регистрации на лекции;
  - `0` отключает автозакрытие.
- Программа курса собирается из БД:
  - шапка (`menu.program_header`) хранится в `graph-src/menu.json`;
  - темы лекций (`lectures.topics`) редактируются в админке во вкладке `Лекции`.
...
Миграция `0008_auto_close_registration` добавляет `lectures.registration_opened_at` и настройку `auto_close_registration_hours=24`.
