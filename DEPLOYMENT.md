...
Автозакрытие регистрации хранится в `app_settings.auto_close_registration_hours`:
- дефолт `24` часа;
- отсчет идет от момента открытия регистрации;
- `0` отключает автозакрытие.
Программа курса формируется из `menu.program_header` + `lectures.topics` (темы лекций редактируются в админке Mini App, вкладка `Лекции`).
...
8. После первого запуска откройте админку Mini App:
   ...
   - вкладка `Настройки` -> при необходимости измените часы автозакрытия регистрации (0..8760, где `0` = выключено);
